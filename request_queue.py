...
class LLMRequestQueue:
    def __init__(self, max_concurrent: int = 6, delay_between_requests: float = 1.0):
        rate_limit_delay = os.getenv("STRIX_RATE_LIMIT_DELAY")
        if rate_limit_delay:
            delay_between_requests = float(rate_limit_delay)
        
        rate_limit_concurrent = os.getenv("STRIX_RATE_LIMIT_CONCURRENT")
        ...
        logger.info(
            f"LLMRequestQueue initialized: delay={delay_between_requests}s, "
